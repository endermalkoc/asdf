...
// FindConfigYAMLPath finds the config.yaml file in .cusp directory
// Walks up from CWD to find .cusp/config.yaml
func FindConfigYAMLPath() (string, error) {
	configPath, err := findProjectConfigYaml()
	if err != nil {
		return "", fmt.Errorf("no .cusp/config.yaml found in current directory or parents")
	}

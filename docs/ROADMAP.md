...
|---|---|---|
| **Remote sync — remaining** | federation (peers) | push/pull/remote/fetch, `cusp sync`, and `cusp dolt clone` are **done** (see [CHANGELOG.md](CHANGELOG.md)). Remaining: federation/peers. |
...
| **Validation & analysis — remaining (low-priority)** | a standalone `cusp migrate`; deeper data-drift; table-shape schema integrity | The suite (`check`, `impact`, `coverage`, `doctor`, `doctor --fix`) is **done** — see [CHANGELOG.md](CHANGELOG.md). Leftovers: graduate `app.MigrateWorkspace` into a standalone **`cusp migrate`** command; **data-drift** checks beyond fr_key; and **schema integrity** that compares the actual table shape to what the migrations expect (not just the version cursor). |
...
| **DB maintenance — remaining** | `cusp backup`/`restore` | History maintenance (`flatten`, `dolt compact`, `dolt gc`) is **done** (see [CHANGELOG.md](CHANGELOG.md)). Remaining: `backup`/`restore`. |

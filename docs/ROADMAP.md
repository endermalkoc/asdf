...
| **Validation & analysis — mostly done** | deeper drift; a standalone `cusp migrate` | **DONE** (see [CHANGELOG.md](CHANGELOG.md)): **`cusp check`** (inline-ref + cycle integrity), **`cusp impact`** (refs + edges + `ent_relationship`), **`cusp coverage`** (rollups + orphan-FR + delivery-status drift), **`cusp doctor`** (integrity + schema drift + coverage + fr_key drift + hygiene, gating exit code), and **`cusp doctor --fix`** (migrates a behind DB; recomputes stale fr_keys). **Remaining (low-priority):** the migrate logic (`app.MigrateWorkspace`) could graduate to a standalone **`cusp migrate`** command; deeper data-drift checks (beyond fr_key); and richer schema-integrity (actual-table-shape vs expected, not just the version cursor). |

...
| **Validation & analysis — remaining** | drift detection; `doctor` auto-fix | **`cusp check`** (inline-ref + cycle integrity), **`cusp impact <ref>`** (refs + edges + `ent_relationship`, `--transitive`), **`cusp coverage`** (requirement→test-case rollups + orphan-FR + delivery-status drift), and **`cusp doctor`** (health aggregator: integrity + coverage + hygiene, gating exit code) are **done** (see [CHANGELOG.md](CHANGELOG.md)). **Remaining:** schema/data **drift detection** (adapt beads patterns; we have `schema.CheckForwardDrift`-style hooks), and an optional **`doctor --fix`** for the auto-fixable findings (most current findings need a human, so this is low-priority). |

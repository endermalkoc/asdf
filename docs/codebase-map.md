...
- **beads** — `/home/ender/repos/misc/beads` (`../../misc/beads` from this repo). The MIT project
  Cusp's Dolt infrastructure was **salvaged and adapted** from; [NOTICE](../NOTICE) lists exactly
  which packages were lifted. When ROADMAP says "adapt beads' X" or "beads has Y", that's where it
  lives. Treat as a read-only reference — Cusp's `internal/` copies have rewritten import paths and
...
| Roadmap item | Start in Cusp | beads reference |
|---|---|---|
